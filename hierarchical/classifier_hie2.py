...
for epoch in range(opt.start_epoch, opt.nEpochs + 1):  
    ...
    for iteration, (auged, label) in enumerate(train_data_loader, 0):
        ...
        ctr_loss = center_loss(species_output, label[:, 0].to('cuda:0')) * centerloss_alpha
        spc_loss = cross_entropy(species_output, label[:, 0].to('cuda:0'))
        gns_loss = cross_entropy(genus_output, label[:, 1].to('cuda:0'))
        fml_loss = cross_entropy(family_output, label[:, 2].to('cuda:0'))
