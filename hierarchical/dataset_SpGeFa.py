...
augseq_special = iaa.Sequential([
    ...
    ,sometimes(iaa.Affine(
            scale={"x": (0.9, 1.1), "y": (0.9, 1.1), "z": (0.9, 1.1), "w": (0.9, 1.1)}, 
            # scale images to 80-120% of their size, individually per axis
            translate_percent={"x": (-0.1, 0.1), "y": (-0.1, 0.1), "z": (-0.1, 0.1), "w": (-0.1, 0.1)}, 
...
class ImageDatasetFromFileSpecial(data.Dataset):
    def __init__(self, image_list, root_path, y=None, z=None, w=None, aug=True, return_filename=False):
    ...
    def __getitem__(self, index):
        ...
        if self.z is None:
          ...
          if self.return_filename:
            return img, self.z[index], self.image_filenames[index]
          else:
        ...
        if self.w is None:
          ...
          if self.return_filename:
            return img, self.w[index], self.image_filenames[index]
          else:
            return img, self.w[index]
